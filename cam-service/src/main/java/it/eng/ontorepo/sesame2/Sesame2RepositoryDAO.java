...
public class Sesame2RepositoryDAO implements RepositoryDAO {
    ...
    /**
     * author ascatox
     * Modified at 20/09/2016
     */
    private static final String QUERY_ALL_DATA_PROPS =
            "SELECT DISTINCT ?name ?type ?range  " +
                    "WHERE { " +
                    "OPTIONAL { ?name rdfs:range ?range } " +
                    "} ";
...
//			if(cn.getNamespace() ==null ||  cn.getNamespace().trim().equals("") || cn.getNamespace().equalsIgnoreCase(getImplicitNamespace()))
    ...
    private PropertyDeclarationItem getPropertyDeclarationItem(BindingSet s) {
        ...
        String type = s.getValue("type") != null ? s.getValue("type").stringValue() : OWL.DATATYPEPROPERTY.stringValue();
    ...
    public void createAssetModel(String name, String className, String ownerName)
        ...
        if (null != ownerName && !ownerName.isEmpty()) {
            ...
            ownerName = Util.getGlobalName(BeInCpps.SYSTEM_NS, ownerName);
